...
def build_distance_matrix(train_data, input_data, max_warping_window, subsample_step):
    # Compute the distance matrix
    distance_matrix_count = 0

    # Compute condensed distance matrix (upper triangle) of pairwise dtw distances
    # when x and y are the same array
    if (np.array_equal(train_data, input_data)):
        x_s = np.shape(train_data)
        distance_matrix = np.zeros((x_s[0] * (x_s[0] - 1)) // 2, dtype=np.double)

        with tqdm(total=x_s[0] * (x_s[0] - 1)) as progress:

            for i in range(0, x_s[0] - 1):
                for j in range(i + 1, x_s[0]):
                    distance_matrix[distance_matrix_count] = dtw_distance(
                        train_data[i, ::subsample_step],
                        input_data[j, ::subsample_step],
    ...
    # Compute full distance matrix of dtw distnces between x and y
        ...
        x_s = np.shape(train_data)
        y_s = np.shape(input_data)
        distance_matrix = np.zeros((x_s[0], y_s[0]))

        with tqdm(total=x_s[0] * y_s[0]) as progress:
